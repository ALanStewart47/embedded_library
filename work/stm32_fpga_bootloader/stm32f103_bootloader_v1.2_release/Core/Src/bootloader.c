 ...
 *				V1.2	2026-09-08		Alan	- Add boot version query (72 68 F0 16 -> 72 68 F0 02 12 16)
 * @note 
 *       1 tab == 4 spaces!
 *
 *****************************************************************************/
...
void ch9121_gpio_init(void)
{
    ...
    LL_GPIO_SetOutputPin(BL_CH9121_CFG_PORT, BL_CH9121_CFG_PIN);
    LL_GPIO_SetOutputPin(BL_CH9121_RST_PORT, BL_CH9121_RST_PIN);

    GPIO_InitStruct.Pin = BL_CH9121_CFG_PIN;
    GPIO_InitStruct.Mode = LL_GPIO_MODE_OUTPUT;
    GPIO_InitStruct.Speed = LL_GPIO_SPEED_FREQ_LOW;
    GPIO_InitStruct.OutputType = LL_GPIO_OUTPUT_PUSHPULL;
    GPIO_InitStruct.Pull = LL_GPIO_PULL_UP;
    LL_GPIO_Init(BL_CH9121_CFG_PORT, &GPIO_InitStruct);

    GPIO_InitStruct.Pin = BL_CH9121_RST_PIN;
    GPIO_InitStruct.Mode = LL_GPIO_MODE_OUTPUT;
    GPIO_InitStruct.Speed = LL_GPIO_SPEED_FREQ_LOW;
    GPIO_InitStruct.OutputType = LL_GPIO_OUTPUT_PUSHPULL;
    GPIO_InitStruct.Pull = LL_GPIO_PULL_UP;
    LL_GPIO_Init(BL_CH9121_RST_PORT, &GPIO_InitStruct);
}
#endif
...
{
    if((uart->rx_length == 4U) && (uart->rx_buffer[0] == 0x72U) &&
       (uart->rx_buffer[1] == 0x68U) && (uart->rx_buffer[2] == 0xF0U) &&
       (uart->rx_buffer[3] == 0x16U))
    {
        uint8_t reply[] =
        {
            0x72U, 0x68U, 0xF0U, 0x02U,
            (uint8_t)((BL_VERSION_MAJOR << 4) | BL_VERSION_MINOR),
            0x16U
        };

        boot_uart_send(uart, reply, sizeof(reply));
        return;
    }
...
static void fpga_send(boot_uart_context_t *uart, unsigned char *data, unsigned char len)
{
    boot_uart_send(uart, data, len);
}

#if BL_ENABLE_ANLOGIC_FPGA
static void anlogic_fpga_start_ack(void)
{
    uint8_t uart_index;
    static const uint8_t ack[] = {0xfaU, 0x68U, 0x01U, 0x16U};

    for(uart_index = 0U; uart_index < BOOT_UART_COUNT; uart_index++)
    {
        boot_uart_send(&boot_uarts[uart_index], ack, sizeof(ack));
    }
}
#endif
...
static void CMD_40_handle(boot_uart_context_t *uart)
{
    ...
    if(length == (spi_w_handle.block_size_t + 7))
    {
...
static unsigned char anlogic_fpga_control_send(uint16_t address, uint32_t data)
{
    uint8_t bit;
