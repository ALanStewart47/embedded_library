...
Assert-True ($bootHeader -match '#define\s+BL_VERSION_MAJOR\s+1U') 'BL_VERSION_MAJOR must be 1 in bootloader.h.'
Assert-True ($bootHeader -match '#define\s+BL_VERSION_MINOR\s+2U') 'BL_VERSION_MINOR must be 2 in bootloader.h.'
Assert-True ($bootHeader -notmatch 'BL_INFO_ADDRESS') 'Flash identity block address must be removed; it overflows IROM.'
Assert-True ($bootHeader -notmatch 'BL_INFO_MAGIC') 'Flash identity magic must be removed.'
Assert-True ($bootSource -notmatch 'bl_info_t') 'bl_info_t must be removed.'
Assert-True ($bootSource -notmatch 'at\(BL_INFO_ADDRESS\)') 'at(BL_INFO_ADDRESS) must be removed.'

$blCmdSource = Get-CFunctionBody $bootSource 'static void BootLoaderCmd_handle'
Assert-True ($blCmdSource.Length -gt 0) 'BootLoaderCmd_handle function body must exist.'
Assert-True ($blCmdSource -match 'rx_length\s*==\s*4U') 'Version query must match a 4-byte frame.'
Assert-True ($blCmdSource -match '0xF0U') 'Version query command byte must be 0xF0.'
Assert-True ($blCmdSource -match '0x72U,\s*0x68U,\s*0xF0U,\s*0x02U') 'Version query reply must be 72 68 F0 02 <ver> 16.'
Assert-True ($blCmdSource -match 'BL_VERSION_MAJOR') 'Version query reply must pack BL_VERSION_MAJOR/MINOR.'

$f0Start = $blCmdSource.IndexOf('rx_length == 4U')
$f0Source = ''
if ($f0Start -ge 0) {
    $f0Brace = $blCmdSource.IndexOf('{', $f0Start)
    if ($f0Brace -ge 0) {
        $depth = 0
        for ($i = $f0Brace; $i -lt $blCmdSource.Length; $i++) {
            $ch = $blCmdSource[$i]
            if ($ch -eq '{') { $depth++ }
            elseif ($ch -eq '}') {
                $depth--
                if ($depth -eq 0) {
                    $f0Source = $blCmdSource.Substring($f0Start, ($i - $f0Start) + 1)
                    break
                }
            }
        }
...
Assert-True ($f0Source.Length -gt 0) 'Version query branch body must exist.'
Assert-True ($f0Source -notmatch 'upgrade_bin_flag\s*=') 'Version query must not take the MCU upgrade session.'
Assert-True ($f0Source -notmatch 'active_uart\s*=') 'Version query must not claim the active UART.'

if ($failures.Count -ne 0) {
    foreach ($failure in $failures) {
        Write-Error $failure -ErrorAction Continue
    }
    Write-Host "Static checks failed: $($failures.Count) / $assertCount"
    exit 1
}

Write-Host "Static checks passed: $assertCount assertions"
