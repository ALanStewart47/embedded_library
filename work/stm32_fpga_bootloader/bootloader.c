...
void bsp_uart_baud_reinit(void )
{
  ...
  /* USER CODE BEGIN USART1_Init 1 */
	#if (URAT_NUMBER == 1)
		RE_UART.Instance = USART1;
	#endif
  
	#if (URAT_NUMBER == 2)
		RE_UART.Instance = USART2;
	#endif

	#if (URAT_NUMBER == 3)
		RE_UART.Instance = USART3;
	#endif

	#if (URAT_NUMBER == 4)
		RE_UART.Instance = UART4;
	#endif

  /* USER CODE END USART1_Init 1 */
	baudrate = bsp_check_baud();
	RE_UART.Init.BaudRate = baudrate;
	RE_UART.Init.WordLength = UART_WORDLENGTH_8B;
	RE_UART.Init.StopBits = UART_STOPBITS_1;
	RE_UART.Init.Parity = UART_PARITY_NONE;
	RE_UART.Init.Mode = UART_MODE_TX_RX;
	RE_UART.Init.HwFlowCtl = UART_HWCONTROL_NONE;
	RE_UART.Init.OverSampling = UART_OVERSAMPLING_16;
	if (HAL_UART_Init(&RE_UART) != HAL_OK)
	{
		Error_Handler();
	}
